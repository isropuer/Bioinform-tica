...
```
git config --global init.defaultBranch soca (Puedes tener varias ramas, git config te muestra en cual estas. Con git branch -a te salen todas las ramas que tienes).
```
...
Es importante no crear repositorios en subcarpetas de un repositorio previo (La creas encima de las carpetas y cosas que quieres que te controle y ordene).
...
```
git commit -m (el mensaje se aplica a todos los archivos que hayas agrupado) "Iniciada la primera práctica, aunque el alineamiento no está hecho todavía."
```
...
*.bam
```
nano .gitignore para abrir todo lo que tienes dentro del git ignore.
