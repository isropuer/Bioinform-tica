---
title: "Práctica 9.2 Reconstrucción filogenética"
...
Nº de secuencias: 42
Longitud del alineamiento: 16693 bases
names(mtdna)
...
**Ejercicio**
**¿De qué clase son los objetos mt_nj y mt_upgma?** Phylo
...
Es igual que plot, solo que tienes muchos parámetros para modificar.
...
add.scale.bar(): Añade una barra horizontal que indica la escala de las longitudes de las ramas a la representación gráfica de un árbol filogenético.
...
plot(mt_nj_rtt)
add.scale.bar(mt_nj_rtt)
#rtt es para enraizar un árbol filogenético con puntas fechadas en la ubicación más compatible con la suposición de un reloj molecular estricto.
```

**Ejercicio**
**¿Se corresponden bien las distancias evolutivas estimadas con las dataciones?**
Se ha añadido la escala a la representación para compararlo con los otros árboles generados. En su mayoría las distancias evolutivas y sus dataciones coinciden, pero hay algunas ramas, como la de los denisovanos muestran una datación mucho más cercana a lo que sus distancias evolutivas manifiestan.
```{r}
#| fig-height: 15
plot(mt_nj_rtt, align.tip.label = TRUE, lwd = 4)
```

**¿Qué secuencias crees que se ajustan menos a su supuesta edad?** La de los denisovanos, parece que son más antiguos que lo que realmente son.
**¿Qué razones puede haber para que la edad de una secuencia no se corresponda con la cantidad de cambios que ha acumulado?** Que la datación no sea del todo correcta o fiable, que se de un rango de datación muy grande, lo que significaría que no hay mucho conocimiento sobre su datación real. Otro explicación sería que hubiera fragmentación del DNA, que indujera muchos más cambios de los esperados.
...
z[z == 0] <- NA
mean(z, na.rm = TRUE)
#Hacemos que todos los valores 0 en la matriz sean NA. Con na.rm = TRUE lo que hacemos es que todos los valores que sean = NA se ignoren
```
...
```
En la gráfica se muestra una correlación de 1:1 entre las distancias estimadas y las distancias cofenéticas, por lo que podemos asumir que ambas son prácticamente idénticas.
...
**Ejercicio**
**¿Serías capaz de crear otro conjunto de datos, mtdna28, que sólo contenga secuencias del género Homo?** En el chunk anterior, con el signo "!" antes de grepl lo que lo grabamos era invertir el valor lógico de todo lo que incluíamos en grepl, por lo que todo lo que estaba contenido en grepl (las secuencias de los neandertales) son eliminadas. Ahora, si no añadimos el "!" y dejamos solo grepl, solo se incluiran las secuencias que metamos dentro del grepl.
```{r}
...
**Ejercicio**
**¿Cuál es la tasa de mutación global estimada?** 1.15857e-08
