...
#Muestra todas las filas que son duplicadas
duplicated(genomas)
```
...
Que la 7 es GCF y la 8 es GCA (el nombre)
...
mv ~/Descargas/data* ~/bin/ (mover todo lo que empiece por data de mis descargas a mi carpeta bin de linux). Más fácil que todo esto, copias el enlace donde quieras que se te descargue y haces wget "el enlace"
chmod (para ejecutar) u+x (darle al usuario permiso de ejecutar) ~/bin/data* (los 2 docs empiezan por data, todo lo que esté en la carpeta que se llame data...)
