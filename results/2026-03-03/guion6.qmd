...
str(P01923)
nombre_ej3 <- P01923[['metadata']][['sequence']]
nombre_ej3
```
...
```
```{r}
protein <- fromJSON(paste0(API, 'protein'))[['proteins']][['uniprot']]
protein

búsqueda <- fromJSON(paste0(API, 'protein/structure'))[['proteins']][['uniprot']]
búsqueda

proporción <- búsqueda / protein * 100
proporción
```
1387422 taxones sin proteína asignada (son tantos probablemente porque entre las proteínas no revisadas han dejado de añadir las de taxones muy parecidos a las que ya están en la base de datos).
...
*zinc finger, C2HC type* (Pfam PF01530)? Hay 15 proteínas no revisadas.
:::
```{r}
Cocodrilos <- fromJSON(paste0(API, 'protein/taxonomy/uniprot/8493/entry/pfam/PF01530'))
Cocodrilos
```
