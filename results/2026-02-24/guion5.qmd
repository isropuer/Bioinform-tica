...
El primer bloque da problemas al no añadir URLencode tras curl_download ni bajar uuna línea, como el segundo sí. URLencode es una función que codifica algunos símbolos de la URL, para que el blooque de código no piense que algunos códigos son espacios u otras cosas. 
El tercer bloque está codificado de forma diferente el portal, result, query... de modo que si en cualquier momento se quieren cambiar los detalles de la búsqueda, se puede cambiar solo el dato que nos interese. Puedo ir ejecutando las diferentes líneas de uno en uno con Ctrl+Enter, por lo que solo me muestra la variable de interés.
En este caso estan los 5 pasos para la búsqueda en ENA. 1- Sobre que haces la búsqueda. 2- Limitar la búsqueda a unos determinados filtros. 3- Que los registros pertenezcan al taxón 6157. 4- que nos devuelva los resultados. 5- cuantos resultados queremos que nos de, si pones 0 te da todos.
...
curl_download('https://www.ebi.ac.uk/ena/portal/api/search?result=assembly&query=tax_tree(6157) AND genome_representation="full"&fields=version,tax_id,scientific_name,last_updated,base_count&limit=0',
              destfile = 'z1.txt')
#Da error puesto que falta la función URLencode
```
...
```
el separador en la tabla es el tabulador, el cual se marca con "\t"
ADH\$base_count para acceder solo a los valores para base_count. Si le añadimos summary nos da un resumen de este. Con esto sabemos las longitudes mínima, máxima, media, cuartiles...
Unique para que nos dé la lista de valores únicos de esa columna, en este caso saber cuantas espécies.
1. ¿Cuántas secuencias hay con el término "alcohol dehydrogenase" en su descripción? 1258
2. ¿Qué longitudes tienen? Lo que ponga en base_count. Entre 44 y 17499 pb, de media 839
3. ¿De cuántas especies diferentes son estas secuencias? Lenght(unique(ADH\$scientific_name))=308
4. ¿Cuándo se publicaron las más recientes? Lo que ponga en first_public max
:::
