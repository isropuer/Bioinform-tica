...
*Ejercicio 1* *¿Cuántas secuencias contiene el archivo?* Como está en formato FASTA, todas las secuencias empiezan por \>, por lo que usamos zgrep -c '\^\>' para encontrar todos los \>
...
*¿Qué longitudes mínima, media y máxima tienen las secuencias?* awk tras zcat ignora cabeceras y calcula longitudes, sumas, min, max. !/\^\>/: Selecciona solo la línea de la secuencia (la que comienza por el símbolo "\>"). length(\$0): Calcula la longitud de esa línea. sum/count: Calcula el promedio al final (END). min/max: Actualiza los valores mínimo y máximo según recorre el archivo.
...
*¿Cuál es el número total de bases?* zcat vuelve a contar los caracteres en que las líneas empiecen con "\>". !/\^\>/: Le dice al terminal: "coge las líneas que empiecen por \>". Esto es perfecto porque en el formato FASTQ, las líneas de las secuencias comienzan por dicho símbolo. sum += length(\$0): Va acumulando la longitud de cada línea en una variable llamada sum. END {print sum}: Una vez que termina de leer todo el archivo, imprime el resultado final.
...
¿Los parámetros corresponden al ensamblaje de un genoma haploide o diploide?* Con la función de hifiasm --n-hap podemos conocer el número de haplotipos y con ello extrapolar si el genoma es haploide o diploide. Por defecto el programa considera que es diploide 
