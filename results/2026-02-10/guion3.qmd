...
modificar nada en la carpeta. ¿Sabrías contar el número de líneas de cada archivo? 1646728
:::
...
- ¿De cuántas secuencias diferentes se compone este genoma de referencia? 3
:::
...
```
zless es el programa navegador de arvhivos fasta
zless y el nombre del archivo.
tar -xf (nombre del archivo) para extraer
wc (word count) contar lineas. Si es wc -l cuenta líneas
gunzip -c para descomprimir archivos fastq (gunzip -c nombre del archivo | (para redirigir a otro programa) wc -l (el programa aquí es contar líneas.)).
Para descomprimir sin más gunzip nombre del archivo, gzip para comprimir de nuevo.
q para salir de zless
grep para interrogar un archivo con una acción regular (una sola palabra). grep ">" +nombre del archivo. (> porque cada secuencia empieza por >). Si queremos que sean las líneas que tengan ese símbolo al principio sería "^>". Una expresión regular es una forma de definir líneas, palabras... una serie de caracteres que tienen algo en común.
